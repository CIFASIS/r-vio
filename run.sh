#!/bin/bash
#
# Perform several R-VIO operations in a Docker container.
...
if [ $DEV_MODE -eq 1 ] ; then
  docker run -it --rm --net=host -v $CURRENT_DIR:/root/catkin_ws/src/r-vio/ \
   rvio:ros-kinetic
fi
...
if [ $VIS_MODE -eq 1 ] ; then
  if [ $DETACHED -eq 1 ] ; then
    docker run -d --net=host \
       -v $CURRENT_DIR/config/:/root/catkin_ws/src/r-vio/config/ \
       -v $CURRENT_DIR/launch/:/root/catkin_ws/src/r-vio/launch/ \
       rvio:ros-kinetic \
       roslaunch rvio $LAUNCH_FILE
  else
    docker run --net=host \
       -v $CURRENT_DIR/config/:/root/catkin_ws/src/r-vio/config/ \
       -v $CURRENT_DIR/launch/:/root/catkin_ws/src/r-vio/launch/ \
...
if [ $BUILD -eq 1 ] ; then
  docker build --rm --tag rvio:ros-kinetic $CURRENT_DIR
fi
